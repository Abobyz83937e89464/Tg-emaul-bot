...
async def monitor_all_emails():
    ...
    while True:
        try:
            ...
            emails_data = supabase.table('email_accounts').select('*').execute()
            
            for email_acc in emails_data.data:
                email_address = email_acc['email']
                user_id = email_acc['user_id']
                
                # Пароль пока заглушка - нужно будет хранить при регистрации
                ...
                for email_msg in new_emails:
                    ...
                    user_data = supabase.table('users').select('telegram_id').eq('id', user_id).execute()
                    if user_data.data:
                        telegram_id = user_data.data[0]['telegram_id']
...
async def send_email_to_user(telegram_id, email_address, email_data):
    try:
        await bot.send_message(
            chat_id=telegram_id,
            text=f"📧 Новое письмо для {email_address}\n\n"
